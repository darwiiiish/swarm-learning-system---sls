...
import { UserPlus, KeyRound, Hash, ShieldAlert, User, Eye, EyeOff } from 'lucide-react';
...
export default function Signup() {
  ...
  const handleSubmit = async (e) => {
    ...
    // Validations
    if (!regnum.trim() || !name.trim() || !password.trim() || !confirmPassword.trim()) {
      setError('All core fields are required.');
      return;
    }

    if (regnum.trim().length < 4) {
    ...
    setLoading(true);
    try {
      await signup(regnum.trim(), name.trim(), password);
      navigate('/explorer');
    } catch (err) {
      setError(err.message || 'Signup failed. Registration number might be in use.');
    } finally {
      setLoading(false);
    }
  };
      ...
      >
        ...
        <div className="flex-center" style={{ flexDirection: 'column', marginBottom: '2rem' }}>
          ...
          <h2 style={{ fontSize: '1.8rem', textAlign: 'center', marginBottom: '0.25rem' }}>
            Establish <span className="text-green">Node</span>
          </h2>
          <p className="text-muted" style={{ fontSize: '0.8rem', textAlign: 'center', fontFamily: 'var(--font-mono)' }}>
            Register profile into the Swarm Collective
          </p>
        </div>
          ...
          <div style={{ width: '100%' }}>
            ...
            <div style={{ position: 'relative' }}>
              ...
              <input
                ...
                type="text"
                className="input-glass"
                placeholder="e.g. Abdelrahman Mamdouh"
                value={name}
        ...
        <div style={{ textAlign: 'center', marginTop: '1.5rem', fontSize: '0.85rem' }}>
          ...
          <Link to="/login" className="text-green" style={{ textDecoration: 'none', fontWeight: '600' }}>
            Portal Auth
          </Link>
        </div>
